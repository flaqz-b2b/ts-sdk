...
[![npm shield](https://img.shields.io/npm/v/ts-sdk)](https://www.npmjs.com/package/ts-sdk)
...
A full reference for this library is available [here](https://github.com/flaqz-b2b/ts-sdk/blob/HEAD/./reference.md).
...
```typescript
import { FlaqzAppClient } from "ts-sdk";
...
```typescript
import { FlaqzAppError } from "ts-sdk";

try {
    await client.customer.createManyCustomers(...);
} catch (err) {
    if (err instanceof FlaqzAppError) {
...
```typescript
const { data, rawResponse } = await client.customer.createManyCustomers(...).withRawResponse();
...
```typescript
import { FlaqzAppClient, logging } from "ts-sdk";
